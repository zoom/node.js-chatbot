...
let createInfo = {
  help(commandStore, name) {
    ...
    let header = { text: title, style: { bold: true } };
    return {
      body: msg,
      header
    };
...
class Client extends utils.Event {
  ...
  parse({ body, headers }) {
    ...
    if (status === false) {
      ...
      let { cmdOption, type, message, eventName, eventFullName, data } = result;
      ...
      if (ifCommand === false) {
        if (config.errorHelp === true) {
          ...
          try {
            ...
            err = e;
            // allCallback({ type:'triggerNoMatchCommand',errorMessage:e});
          }
          // return this;
        } else {
          // allCallback(null, sendData);
          this.trigger('messages', sendData);
          // return this;
        }
      }
      try {
        await this.trigger('command', { data, type, command: eventName }); //old trigger
  ...
  handleCompatibility({ body, headers }, allCallback) {
    if (typeof body !== 'object') {
      //only check body
      if (typeof allCallback === 'function') {
        allCallback({ type: 'format', errorMessage: 'body format error' });
      }
      return this;
    }
    ...
    if (routeEntity.status === false) {
      if (routeEntity.errorMessage && typeof allCallback === 'function') {
        allCallback({ type: 'verify', errorMessage: routeEntity.errorMessage });
      }
      return this;
    }
